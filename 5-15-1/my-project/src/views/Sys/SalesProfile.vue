    ...
    <!-- 表格内容栏 -->
      ...
      <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="addDialogVisible" :close-on-click-modal="false">
        <el-form :model="addForm" label-width="80px" :rules="dataFormRules" ref="dataForm">
          ...
          <el-form-item label="销售额" prop="userSale">
            <el-input v-model="addForm.userSale" auto-complete="off"></el-input>
          </el-form-item>
      ...
      <el-dialog :title="operation?'新增':'编辑'" width="40%" :visible.sync="editDialogVisible" :close-on-click-modal="false">
        <el-form :model="editForm" label-width="80px" :rules="dataFormRules" ref="dataForm">
          ...
          <el-form-item label="销售额" prop="userSale">
            <el-input v-model="editForm.userSale" auto-complete="off"></el-input>
          </el-form-item>
          <el-form-item label="已结款额" prop="amountAlready">
  ...
  export default {
    ...
    methods: {
      ...
      deleteRow(index, row) {
        ...
        console.log(id)
        this.$api.inte.delProductSale(id).then((res)=>{
          // row.splice(index, 1);
          ...
          this.$refs.dataForm.validate(valid => {
            if (valid) {
              let productInfo = {
                    ...
                    userSale:this.addForm.userSale,
                    userDate:this.addForm.userDate,
                    userid:JSON.parse(sessionStorage.user).userid,
                    orderUnit:this.addForm.orderUnit,
              ...
              this.$api.inte.addProductSale(productInfo).then((res)=>{
                // var user = 
